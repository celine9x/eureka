...
var _UniversalStore = class _UniversalStore2 {
  constructor(options, environmentOverrides) {
    ...
    this.subscribe = (eventTypeOrListener, maybeListener) => {
      ...
      return this.listeners.has(eventType) || this.listeners.set(eventType, /* @__PURE__ */ new Set()), this.listeners.get(eventType).add(listener), () => {
        var _a;
        this.debug("unsubscribe", { eventType, listener }), this.listeners.has(eventType) && (this.listeners.get(eventType).delete(listener), ((_a = this.listeners.get(eventType)) == null ? void 0 : _a.size) === 0 && this.listeners.delete(eventType));
      };
    };
    this.send = (event) => {
  ...
  untilReady() {
    var _a;
    return Promise.all([_UniversalStore2.preparation.promise, (_a = this.syncing) == null ? void 0 : _a.promise]);
  }
  /** Creates a new instance of UniversalStore */
  static create(options) {
    if (!options || typeof (options == null ? void 0 : options.id) != "string")
  ...
  emitToChannel(event, eventInfo) {
    var _a;
    this.debug("emitToChannel", { event, eventInfo, channel: !!this.channel }), (_a = this.channel) == null ? void 0 : _a.emit(this.channelEventName, {
      event,
      eventInfo
    });
  }
  ...
  handleChannelEvents(channelEvent) {
    ...
    if ([eventInfo.actor.id, (_a = eventInfo.forwardingActor) == null ? void 0 : _a.id].includes(this.actor.id)) {
      this.debug("handleChannelEvents: Ignoring event from self", { channelEvent });
      return;
    } else if (((_b = this.syncing) == null ? void 0 : _b.state) === ProgressState.PENDING && event.type !== _UniversalStore2.InternalEventType.EXISTING_STATE_RESPONSE) {
      ...
      switch (event.type) {
        case _UniversalStore2.InternalEventType.EXISTING_STATE_RESPONSE:
          if (this.debug("handleChannelEvents: Setting state from leader's existing state response", {
            event
          }), ((_c = this.syncing) == null ? void 0 : _c.state) !== ProgressState.PENDING)
            break;
          (_e = (_d = this.syncing).resolve) == null ? void 0 : _e.call(_d);
          let setStateEvent = {
            type: _UniversalStore2.InternalEventType.SET_STATE,
            payload: {
              state: event.payload,
...
var getEventSourceUrl = (event, refId) => {
  ...
  let candidates = Array.from(
    ...
    var _a2, _b;
    try {
      return ((_a2 = element.contentWindow) == null ? void 0 : _a2.location.origin) === event.source.location.origin && ((_b = element.contentWindow) == null ? void 0 : _b.location.pathname) === event.source.location.pathname;
    } catch {
    }
    try {
  ...
  }), src = (_a = pickFrameByRefId(candidates, refId)) == null ? void 0 : _a.getAttribute("src");
  if (src) {
...
var PostMessageTransport = class {
  constructor(config) {
    ...
    if (this.buffer = [], typeof ((_a = scope) == null ? void 0 : _a.addEventListener) == "function" && scope.addEventListener("message", this.handleEvent.bind(this), false), config.page !== "manager" && config.page !== "preview")
  ...
  send(event, options) {
    let {
      ...
      ...eventOptions
    }, frames = this.getFrames(target), query = new URLSearchParams((location == null ? void 0 : location.search) || ""), data = stringify(
      {
        key: KEY,
        event,
...
//# sourceMappingURL=chunk-EDBTOAYE.js.map
